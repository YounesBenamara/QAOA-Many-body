...
# --- Shots ---
SHOTS = 4096 # Doesn't matter for ideal QPU
...
# --- Runtime options ---
USE_IDEAL_QPU = False        # Noiseless simulation
USE_RESILIENCE_LEVEL_1 = False # set the level of error mitigation but with extra overhead

# --- Initialization ---
INIT_MODE = "resume"   # "random" for random parameters or "resume" for optimized parameters
INIT_STD_DEV = 1     # standard deviation of the normal distribution for random initial parameters
...
def main():
    if USE_IDEAL_QPU:
        run_name = f"tfim_vqe_ideal_N{N}_g{g}_L{N_LAYERS}_fake_marrakesh"
    else:
        run_name = f"tfim_vqe_noisy_N{N}_g{g}_L{N_LAYERS}_fake_marrakesh"
    ...
    result = run_vqe_spsa_decay_hardware(ansatz_isa=ansatz_isa, hamiltonian_isa=hamiltonian_isa,
                                         ...
                                         plateau_z=PLATEAU_Z, plateau_patience=PLATEAU_PATIENCE,
                                         tail_window=TAIL_WINDOW, enable_early_stop=ENABLE_EARLY_STOP,
                                         resume_history=(INIT_MODE == "resume"), seed=SEED)
    t1 = time.time()
